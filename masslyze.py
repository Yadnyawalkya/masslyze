...
# sslv3, sslv2, CRIME, heartbleed, client-init. reneg, certificate (hostname, trust, keysize, date, weak public key signature algorithm), rc4, weak/medium cipher
        ...
        try:
            for vulnerability in target:
                ...
                if vulnerability.tag == "sslv3":
                    vulnerability_checks.check_for_sslv3(db_cursor, db_connection, host_ip, host_port, vulnerability)
                # sslv2 supported
                if vulnerability.tag == "sslv2":
                    vulnerability_checks.check_for_sslv2(db_cursor, db_connection, host_ip, host_port, vulnerability)
                # CRIME (compression)
                if vulnerability.tag == "compression":
                    vulnerability_checks.check_for_crime(db_cursor, db_connection, host_ip, host_port, vulnerability)
                # heartbleed
                if vulnerability.tag == "heartbleed":
                    vulnerability_checks.check_for_heartbleed(db_cursor, db_connection, host_ip, host_port, vulnerability)
                # reneg
                if vulnerability.tag == "reneg":
                    vulnerability_checks.check_for_reneg(db_cursor, db_connection, host_ip, host_port, vulnerability)
                # certificate
                if vulnerability.tag == "certinfo":
                    vulnerability_checks.check_for_certificate(db_cursor, db_connection, host_ip, host_port, vulnerability)
                #  weak/medium keysize cipher support
                if vulnerability.tag == "sslv3" or "tlsv1" or "tlsv1_1" or "tlsv1_2":
                    vulnerability_checks.check_for_weakmedium_cipher_keysize(db_cursor, db_connection, host_ip, host_port, vulnerability)
                # rc4 supported
                if vulnerability.tag == "sslv3" or "tlsv1" or "tlsv1_1" or "tlsv1_2":
                    vulnerability_checks.check_for_rc4_support(db_cursor, db_connection, host_ip, host_port, vulnerability)
                db_connection.commit()
        except:
...
def generate_outputfile():
    ...
    with open('output.txt', 'w') as outputfile:
        ...
        output_generation.generate_output_by_vulns(db_cursor, db_connection, outputfile)

        output_generation.attach_invalidtargets_to_output(db_cursor, db_connection, outputfile)
    db_connection.close()
